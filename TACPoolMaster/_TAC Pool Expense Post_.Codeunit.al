codeunit 50275 "TAC Pool Expense Post"
{
    ...
    var NoLinesErr: Label 'Expense %1 has no detail lines to post.', Comment = '%1 = Expense ID';
    ...
    begin
        if PoolExpenseHeader.Posted then Error(AlreadyPostedErr, PoolExpenseHeader."Expense ID");
        ...
        if not PoolExpenseDetail.FindSet()then Error(NoLinesErr, PoolExpenseHeader."Expense ID");
        repeat WriteEntry(PoolExpenseHeader, PoolExpenseDetail);
    ...
    begin
        ...
        if not PoolLedgerEntry.IsEmpty()then exit;
