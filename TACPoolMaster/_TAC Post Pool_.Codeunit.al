/*
The close is a single atomic routine. Every step runs in one database transaction; any failure rolls the whole close back.
Parameters: Pool Code, Close Sequence No. (resolved from `T-16`).
Final close
Resolve the schedule row where `Is Final = true`. Reject if a prior sequence has not run.
Reverse the outstanding provisional batch.
Repeat steps 4–8 above with `Provisional = false`, paying the full residual (`C-07`).
Post the final G/L journal.
Post the grower Purchase Invoice, creating the vendor ledger entry the payment run clears and the ABA file pays.
Set `Status = Closed`, stamp `Closed DateTime` and `Closed By`.

Reversal of a final close is permission-restricted (`V-08`) and requires a reason code. Once a pool is finally closed, TAC has made the final grower payment and bears any subsequent adjustment.
Grower Purchase Invoice lines
Line	Content
One per pool	Variety, kilograms and rate per kilogram, valued at the grower's share
One per above-the-line charge	Negative. Omitted where `Suppress from Grower Invoice` is set. Repack charges are not suppressed (`D-07`).
One per prior provisional (final close only)	Negative, so the net is the residual payable
Pool payments are GST-free (fresh avocados are basic food under Australian GST law). Charges carry GST from their posting groups and standard tax calculation applies per line.
...
codeunit 50201 "TAC Post Pool"
{
    ...
    begin
        ...
        if ConsignmentLine.FindSet() then
            repeat
                EstimatedAmount := ConsignmentLine.EstimatedAmount();
                if EstimatedAmount = 0 then
                    continue;
    ...
    local procedure ResolvePoolCloseChargeTypeCode(TransTypeCode: Code[10]): Code[20]
    var
    ...
    begin
        ...
        if not ChargeType.FindFirst() then
            Error(PoolCloseChargeTypeMapErr, TransTypeCode);

        exit(ChargeType.Code);
    end;

    local procedure GetLastPoolLedgerEntryNo(): Integer
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    begin
        ...
        if PoolLedgerEntry.FindSet() then
            repeat
                PostPoolCloseLedgerToGL(PoolLedgerEntry);
            until PoolLedgerEntry.Next() = 0;
    end;
    ...
    begin
        ...
        if PoolLedgerEntry."Source Document No." <> '' then
            GenJournalLine.Validate("Document No.", PoolLedgerEntry."Source Document No.");
    ...
    local procedure MapGrowerType(PoolType: Enum "TAC Grower Pool Type"): Enum "TAC Grower Type"
    begin
        case PoolType of
            PoolType::External:
                exit(Enum::"TAC Grower Type"::External);
            PoolType::"Contract Pack":
    ...
    begin
        ...
        */
        ...
        if Pool."Payment Model" = Pool."Payment Model"::"Full Payout" then
            CumulativePct := 100;
        ...
        if (Growers.Count() = 0) or (PoolKg = 0) then begin
            if IsFinal and (Abs(PoolPayment) > FinalResidualTolerance) then
                Error(NoGrowerKgErr, Pool."Pool Code", PoolPayment);
            exit;
        end;
        ...
        for Idx := 1 to Growers.Count() do begin
            ...
            if GrowerCode <> ResidualGrowerCode then begin
                ...
                if PoolGroup."Pool Group Code" <> '' then
        ...
        if IsFinal then begin
            ...
            if Abs(UnallocatedResidual) > FinalResidualTolerance then
                Error(FinalResidualErr, Pool."Pool Code", UnallocatedResidual, FinalResidualTolerance);
        end;
    ...
    begin
        ...
        if IsFinal then begin
            ...
            if not Schedule.FindFirst() then
                Error(FinalScheduleRowNotFoundErr, Format(ToSchedulePoolType(PoolGroup."Grower Pool Type")));
            exit;
        end;

        Schedule.SetRange("Sequence No.", CloseSequenceNo);
        if not Schedule.FindFirst() then
            Error(ScheduleRowNotFoundErr, Format(Schedule."Pool Type"), CloseSequenceNo);
        if Schedule."Is Final" then
            Error(ScheduleRowFinalErr, CloseSequenceNo, Format(Schedule."Pool Type"));
    end;
    ...
    begin
        ...
        Schedule.SetRange("Is Final", true);
        if not Schedule.FindFirst() then
            Error(FinalScheduleRowNotFoundErr, Format(SchedulePoolType));
    end;
    ...
    begin
        ...
        if CloseSequenceNo <> ExpectedSequenceNo then
            Error(FinalSequenceErr, CloseSequenceNo, PoolGroup."Pool Group Code", ExpectedSequenceNo);
        if CloseSequenceNo <> FinalSequenceNo then
            Error(FinalSequenceErr, CloseSequenceNo, PoolGroup."Pool Group Code", FinalSequenceNo);
        ...
        if not ConsignmentLine.IsEmpty then
            Error(ErrConsignmentLineNotInvoiced, Pool."Pool Code");
    end;

    local procedure GetRealisedNet(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    local procedure GetAlreadyPaid(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetPoolKgs(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetGrowerKgs(PoolCode: Code[20]; GrowerCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure ResidualGrowerForPool(PoolCode: Code[20]; Growers: List of [Code[20]]): Code[20]
    var
    ...
    begin
        ...
        for Idx := 1 to Growers.Count() do begin
            ...
            if (CandidateGrowerCode = '') or
               (GrowerKg > CandidateKg) or
               ((GrowerKg = CandidateKg) and (GrowerCode < CandidateGrowerCode)) then begin
    ...
    local procedure GetFinalResidualTolerance(): Decimal
    var
        PoolSetup: Record "TAC Pool Setup";
    begin
    ...
    begin
        ...
        if not TransType.Get(PoolLedgerEntry."Trans Type Code") then
            exit;

        if Pool."Pool Type" = Pool."Pool Type"::External then
    ...
    begin
        ...
        */
        PoolSetup.Get();
        If PoolSetup."Allow Prov. with Unpriced KG" then
            exit;
        ConsignmentLine.SetRange("Pool Code", PoolCode);
        if ConsignmentLine.FindSet() then
            repeat
                // A fruit payment is the source of truth for whether the consignment is priced.
                Consignment.Get(ConsignmentLine."Consignment No.");
                if not Consignment."Fruit Payment Exists" then
                    Error('Unpriced consignment line %1 in pool %2. Cannot provisionally close.', ConsignmentLine."Line No.", PoolCode);
            until ConsignmentLine.Next() = 0;
    end;


    local procedure ResolveProvisionalSchedule(var Pool: Record "TAC Pool";
                                                CloseSequenceNo: Integer;
                                                var Schedule: Record "TAC Pool Payment Schedule")
    var
        PoolGroup: Record "TAC Pool Group Header";
    begin
        PoolGroup.Get(Pool."Pool Group ID");
    ...
    begin
        ...
        ExpectedSequenceNo := PoolGroup."Provisional Close Count" + 1;
        if CloseSequenceNo <> ExpectedSequenceNo then
            Error(SequenceOutOfOrderErr, CloseSequenceNo, PoolGroup."Pool Group Code", ExpectedSequenceNo);
    end;

    local procedure ToSchedulePoolType(GrowerPoolType: Enum "TAC Grower Pool Type"): Option Internal,External,Grower
    var
        SchedulePoolType: Option Internal,External,Grower;
    begin
        case GrowerPoolType of
            GrowerPoolType::Internal:
                exit(SchedulePoolType::Internal);
            GrowerPoolType::External:
                exit(SchedulePoolType::External);
            else
    ...
    var
        ScheduleRowNotFoundErr: Label 'No active payment schedule row exists for pool type %1 and sequence %2.', Comment = '%1 = Pool Type, %2 = Sequence No.';
