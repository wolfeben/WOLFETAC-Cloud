codeunit 50285 "TAC Pool Consumption Grower"
{
    ...
    var
        MixedGrowerErr: Label 'Production order %1 consumes lots from more than one grower (%2 and %3). A production order can carry only one Grower Code dimension.', Comment = '%1 = Production Order No., %2 = first Grower Code, %3 = second Grower Code';
    ...
    var
        ...
        GrowerCode: Code[20];
    begin
        if not DimensionMgt.DimensionSetupComplete() then
            exit;
        ...
        if ItemLedgerEntry."Order Type" <> ItemLedgerEntry."Order Type"::Production then
            exit;
        if (ItemLedgerEntry."Order No." = '') or (ItemLedgerEntry."Lot No." = '') then
            exit;
        if not IsPoolProductionOrder(ItemLedgerEntry."Order No.", DimensionMgt) then
            exit;
        if not FindReceiptGrower(ItemLedgerEntry, GrowerCode) then
            exit;
    ...
    local procedure IsPoolProductionOrder(ProductionOrderNo: Code[20]; var DimensionMgt: Codeunit "TAC Pool Dimension Mgt"): Boolean
    var
    ...
    begin
        ...
        if not ProductionOrder.FindFirst() then
            exit(false);
        if DimensionMgt.HasPoolDimensions(ProductionOrder."Dimension Set ID") then
            exit(true);
        ...
        if ProductionOrderLine.FindSet() then
            repeat
                if DimensionMgt.HasPoolDimensions(ProductionOrderLine."Dimension Set ID") then
                    exit(true);
            until ProductionOrderLine.Next() = 0;
        exit(false);
    end;

    local procedure FindProductionOrderGrower(ProductionOrderNo: Code[20]; CurrentGrowerCode: Code[20]) GrowerCode: Code[20]
    var
    ...
    begin
        ...
        if ConsumptionItemLedgerEntry.FindSet() then
            repeat
                if FindReceiptGrower(ConsumptionItemLedgerEntry, ConsumedGrowerCode) then
                    if ConsumedGrowerCode <> GrowerCode then
                        Error(MixedGrowerErr, ProductionOrderNo, GrowerCode, ConsumedGrowerCode);
            until ConsumptionItemLedgerEntry.Next() = 0;
    end;

    local procedure FindReceiptGrower(ConsumptionItemLedgerEntry: Record "Item Ledger Entry"; var GrowerCode: Code[20]): Boolean
    var
    ...
    begin
        ...
        repeat
            if ReceiptItemLedgerEntry.Get(ItemApplicationEntry."Inbound Item Entry No.") then
                if (ReceiptItemLedgerEntry."Entry Type" = ReceiptItemLedgerEntry."Entry Type"::Purchase) and
                   (ReceiptItemLedgerEntry."Source Type" = ReceiptItemLedgerEntry."Source Type"::Vendor) then begin
    ...
    local procedure GrowerCodeForVendor(VendorNo: Code[20]): Code[20]
    var
    ...
    begin
        if not PoolSetup.Get() then
            Error('Pool Payment Setup must be configured before consumption can resolve a grower.');
        GrowerDimensionCode := PoolSetup."Grower Dimension Code";
        if GrowerDimensionCode = '' then
            Error('Grower Dimension Code must be configured in Pool Payment Setup before consumption can resolve a grower.');
        if not DefaultDimension.Get(Database::Vendor, VendorNo, GrowerDimensionCode) then
            Error('Vendor %1 has no %2 default dimension. Set the Grower Code on the vendor card before consuming the received lot.', VendorNo, GrowerDimensionCode);

        GrowerCode := DefaultDimension."Dimension Value Code";
        if (GrowerCode = '') or not DimensionValue.Get(GrowerDimensionCode, GrowerCode) then
            Error('Vendor %1 has an invalid %2 default dimension value.', VendorNo, GrowerDimensionCode);
        exit(GrowerCode);
    end;
    ...
    begin
        ...
        if ProductionOrderLine.FindSet() then
            repeat
                UpdateProductionOrderLineDimension(ProductionOrderLine, GrowerCode, GrowerDimensionCode);
            until ProductionOrderLine.Next() = 0;
    end;
    ...
    begin
        ...
        if ProductionOrder."Dimension Set ID" = NewDimensionSetID then
            exit;
    ...
    begin
        ...
        if ProductionOrderLine."Dimension Set ID" = NewDimensionSetID then
            exit;
        ProductionOrderLine.Validate("Dimension Set ID", NewDimensionSetID);
        ProductionOrderLine.Modify(true);
    end;

    local procedure DimensionSetWithGrower(OldDimensionSetID: Integer; GrowerDimensionCode: Code[20]; GrowerCode: Code[20]) NewDimensionSetID: Integer
    var
