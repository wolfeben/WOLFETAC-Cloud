codeunit 50282 "TAC Pool Post Diagnostic"
{
    ...
    var
        HeadingTxt: Label 'Pool posting diagnostic for %1', Comment = '%1 = Production Order No.';
    ...
    procedure Explain(ProductionOrderNo: Code[20]): Text
    var
    ...
    begin
        ...
        case true of
            AlreadyPooled:
                Builder.AppendLine(VerdictPooledTxt);
            Blocker <> '':
                Builder.AppendLine(StrSubstNo(VerdictBlockedTxt, Blocker));
            else
    ...
    local procedure AppendChecks(var Builder: TextBuilder; var ProductionOrder: Record "Production Order"; var AlreadyPooled: Boolean) Blocker: Text
    var
    ...
    begin
        ...
        if OutputCount = 0 then
            exit(StrSubstNo(NoOutputAtAllFix, Prefix));
        ...
        if not IsPacking then
            exit(StrSubstNo(NotPackedFix, Prefix));
        ...
        if not UomReady then
            exit(NoUomSetupFix);
        if not DimensionsReady then
            exit(NoDimensionSetupFix);
    ...
    local procedure AppendQuantities(var Builder: TextBuilder; ProductionOrderNo: Code[20]) Blocker: Text
    var
        ProdOrderPost: Codeunit "TAC Pool Prod Order Post";
    ...
    begin
        ...
        if Units <= 0 then
            exit(NoUnitsFix);
        if Bins <= 0 then
            exit(StrSubstNo(NoBinsFix, ProdOrderPost.BinUoMCode()));
        exit('');
    end;

    local procedure AppendDimensions(var Builder: TextBuilder; var ProductionOrder: Record "Production Order") Blocker: Text
    var
    ...
    begin
        ...
        Builder.AppendLine(StrSubstNo(DimensionsTxt, //Blank(PoolWeekCode), 
            Blank(VarietyCode), Blank(GradeCode), Blank(SizeCode),
            Blank(GrowerCode),
            Format(GrowerPoolType)));
        ...
        if not GrowerMgt.FindVendorNoForGrower(GrowerCode, VendorNo) then
            exit(StrSubstNo(NoGrowerVendorFix, GrowerCode));
        exit('');
    end;

    local procedure FindOrder(ProductionOrderNo: Code[20]; var ProductionOrder: Record "Production Order"): Boolean
    begin
    ...
    local procedure DimensionSetupComplete(): Boolean
    var
        DimensionMgt: Codeunit "TAC Pool Dimension Mgt";
    begin
    ...
    local procedure YesNo(Value: Boolean): Text
    begin
        if Value then
            exit(YesTxt);
        exit(NoTxt);
    ...
    local procedure Blank(Value: Code[20]): Text
    begin
        if Value = '' then
            exit('(blank)');
        exit(Value);
