table 50230 "TAC Pool Group Header"
{
    ...
    var
        NotOpenErr: Label 'Pool Group %1 is not Open; adjustments and expenses are not allowed.', Comment = '%1 = Pool Group Code';
    ...
    begin
        if Status <> Status::Open then
            Error(NotOpenErr, "Pool Group Code");
    end;
    ...
    /// </summary>
    procedure FindOrCreate(NewPoolWeekCode: Code[10]; NewGrowerPoolType: Enum "TAC Grower Pool Type"): Integer
    var
    ...
    begin
        ...
        if NewPoolWeekCode = '' then
            Error(BlankPoolWeekErr);
        if not PoolWeek.Get(NewPoolWeekCode) then
            Error(UnknownPoolWeekErr, NewPoolWeekCode);
    ...
    local procedure BuildGroupCode(var PoolWeek: Record "TAC Pool Week"; GrowerPoolType: Enum "TAC Grower Pool Type"): Code[20]
    var
        WeekText: Text;
    begin
        WeekText := Format(PoolWeek."Week No.");
        if StrLen(WeekText) < 2 then
            WeekText := '0' + WeekText;
        exit(CopyStr(StrSubstNo('PG-%1-W%2-%3', PoolWeek."Season Code", WeekText, TypeLetter(GrowerPoolType)), 1, MaxStrLen("Pool Group Code")));
    end;

    local procedure BuildGroupDescription(var PoolWeek: Record "TAC Pool Week"; GrowerPoolType: Enum "TAC Grower Pool Type"): Text[100]
    begin
    ...
    local procedure TypeLetter(GrowerPoolType: Enum "TAC Grower Pool Type"): Text[1]
    begin
