codeunit 50270 "TAC Pool Charge Engine"
{
    ...
    var
        TrayPackTypeCategoryTok: Label 'TRAY', Locked = true;
    ...
    begin
        ...
        if Evaluation.FindSet() then
            repeat
                // Each Trans Type in scope is resolved once. Iterating the
                ...
                case Evaluation.Result of
                    ...
                    Evaluation.Result::"No Template":
                        if Evaluation.Mandatory then
                            Error(MissingMandatoryTemplateErr, Evaluation."Trans Type Code");
                    Evaluation.Result::"No Match":
                        if Evaluation.Mandatory then
                            Error(NoMatchMandatoryErr, Evaluation."Trans Type Code");
                    Evaluation.Result::Ambiguous:
                        Error(EqualSpecificityErr, Evaluation."Trans Type Code", Ctx."Grower Code");
    ...
    begin
        ...
        if TransType.FindSet() then
            repeat
                HasTemplate := HasActiveNonBinTemplate(TransType.Code, ChargeAction);
        ...
        if not HasTemplate then begin
            ...
            if IsAmbiguous then begin
                ...
                if not HasWinner then begin
                    ...
                    if not TemplateDefinitionIsValid(Winner, TransType, Result.Details) then
                        Result.Result := Result.Result::Invalid
                    else
                        if Winner."Rate Source" = Winner."Rate Source"::Calculated then begin
    ...
    local procedure HasActiveNonBinTemplate(TransTypeCode: Code[10]; ChargeAction: Enum "TAC Pool Charge Action"): Boolean
    var
        Template: Record "TAC Pool Charge Template";
    begin
    ...
    local procedure TemplateDefinitionIsValid(var Template: Record "TAC Pool Charge Template"; var TransType: Record "TAC Pool Trans Type"; var Details: Text[250]): Boolean
    begin
        if (Template."Rate Type" = TransType."Charge Rate Type") and
           (Template."Rate Source" = TransType."Rate Source") then
            exit(true);
        Details := StrSubstNo(RateDefinitionErr, Template.ID, TransType.Code);
        exit(false);
    end;

    local procedure NextEvaluationEntryNo(var Result: Record "TAC Pool Charge Evaluation" temporary): Integer
    begin
        if Result.FindLast() then
            exit(Result."Entry No." + 1);
        exit(1);
    end;

    local procedure ProcessTransType(TransTypeCode: Code[20];
    ChargeAction: Enum "TAC Pool Charge Action"; var Ctx: Record "TAC Pool Charge Context" temporary; Mode: Enum "TAC Pool Charge Mode")
    var
        Winner: Record "TAC Pool Charge Template";
        TransType: Record "TAC Pool Trans Type";
        Found: Boolean;
    ...
    begin
        ...
        if not Found then begin
            ...
            if TransType.Mandatory then
                Error(NoMatchMandatoryErr, TransTypeCode);
            exit;
        end;

        // Calculated (FR) rate is computed in the close (F-07 Step 2) and is
        // blocked by OI-04; the engine never writes an FR amount here.
        if (Winner."Rate Source" = Winner."Rate Source"::Calculated) or
           (Winner."Rate Type" = Winner."Rate Type"::Bin) then
            exit;
    ...
    /// </summary>
    local procedure FindWinningRow(TransTypeCode: Code[20];
                                   ChargeAction: Enum "TAC Pool Charge Action";
                                   var Ctx: Record "TAC Pool Charge Context" temporary;
                                   var Winner: Record "TAC Pool Charge Template";
                                   var IsAmbiguous: Boolean): Boolean
    var
        Template: Record "TAC Pool Charge Template";
        HaveWinner: Boolean;
        RowCount: Integer;
    ...
    begin
        ...
        if Template.FindSet() then
            repeat
                if (Template."Rate Type" <> Template."Rate Type"::Bin) and IsEligible(Template, Ctx) then begin
                    ...
                    if not HaveWinner then begin
                        ...
                        if RowCount > WinnerCount then begin
                            Winner := Template;
                            WinnerCount := RowCount;
                            WinnerHasGrower := RowHasGrower;
                            IsAmbiguous := false;
                        end else
                            if RowCount = WinnerCount then
                                case true of
                                    RowHasGrower and not WinnerHasGrower:
                                        begin
                                            Winner := Template;
                                            WinnerHasGrower := true;
                                            IsAmbiguous := false;
                                        end;
                                    WinnerHasGrower and not RowHasGrower:
                                        IsAmbiguous := false; // keep current winner
                                    else
    ...
    local procedure IsEligible(var Template: Record "TAC Pool Charge Template"; var Ctx: Record "TAC Pool Charge Context" temporary): Boolean
    var
        RipenerFilter: Integer;
    begin
        // Eligibility (§6.4.1): every non-blank filter must match exactly.
        if not MatchCode(Template."Supplier Type Filter", SupplierTypeLetter(Ctx."Supplier Type")) then
            exit(false);
        if not MatchCode(Template."Grower Type Filter", GrowerTypeLetter(Ctx."Grower Type")) then
            exit(false);
        if not MatchCode(Template."Variety Filter", Ctx."Variety Code") then
            exit(false);
        if not MatchCode(Template."Pack Type Filter", Ctx."Pack Type Code") then
            exit(false);
        if not MatchCode(Template."Pack Type Category Filter", Ctx."Pack Type Category Code") then
            exit(false);
        if not MatchCode(Template."Grade Filter", Ctx."Grade Code") then
            exit(false);
        if not MatchCode(Template."Grower Code Filter", Ctx."Grower Code") then
            exit(false);
        ...
        case RipenerFilter of
            1:
                if not RipenerConditionMet(Ctx) then
                    exit(false);
            2:
                if RipenerConditionMet(Ctx) then
                    exit(false);
        end;

        // Grade exclusion (50223) rejects an otherwise-eligible row.
        if GradeExcluded(Template.ID, Ctx."Grade Code") then
            exit(false);

        exit(true);
    end;

    local procedure NonBlankFilterCount(var Template: Record "TAC Pool Charge Template"): Integer
    var
        Count: Integer;
    begin
        if Template."Supplier Type Filter" <> '' then
            Count += 1;
        if Template."Grower Type Filter" <> '' then
            Count += 1;
        if Template."Variety Filter" <> '' then
            Count += 1;
        if Template."Pack Type Filter" <> '' then
            Count += 1;
        if Template."Pack Type Category Filter" <> '' then
            Count += 1;
        if Template."Grade Filter" <> '' then
            Count += 1;
        if Template."Grower Code Filter" <> '' then
            Count += 1;
        if Template."Ripener Required Filter" <> Template."Ripener Required Filter"::" " then
            Count += 1;
        exit(Count);
    end;

    local procedure MatchCode(FilterValue: Code[20]; ContextValue: Code[20]): Boolean
    begin
    ...
    local procedure GradeExcluded(TemplateID: Integer; GradeCode: Code[10]): Boolean
    var
    ...
    local procedure RipenerConditionMet(var Ctx: Record "TAC Pool Charge Context" temporary): Boolean
    var
    ...
    begin
        ...
        if Ctx."Customer No." = '' then
            exit(false);
        if not Ship2Address.Get(Ctx."Customer No.", Ctx."DC Code") then
            exit(false);
    ...
    begin
        ...
        if Mode = "TAC Pool Charge Mode"::Validate then
            exit; // dry-run: rate + amount resolved, nothing written (ADR-001).

        if ChargeAlreadyPosted(Ctx."Pool Code", Template."Trans Type Code", Ctx."Source Document No.", Ctx."Source Line No.") then
            exit;
        ...
        if Ctx."Grower Code" <> '' then
            GrowerNo := GrowerMgt.VendorNoForGrower(Ctx."Grower Code");
    ...
    begin
        ...
        case Ctx."Source Type" of
            Ctx."Source Type"::"Production Output":
                PoolLedgerEntry."Document Type" := PoolLedgerEntry."Document Type"::"Run Close";
            Ctx."Source Type"::"Sales Invoice", Ctx."Source Type"::"Sales Credit Memo":
                PoolLedgerEntry."Document Type" := PoolLedgerEntry."Document Type"::"Sales Invoice";
            Ctx."Source Type"::Consignment:
                begin
        ...
        if TransType.FindSet() then
            repeat
                ...
                if Template.IsEmpty() then
                    Error(MissingMandatoryTemplateErr, TransType.Code);
            until TransType.Next() = 0;
    end;
    ...
    begin
        if (Template."Rate Type" <> TransType."Charge Rate Type") or
           (Template."Rate Source" <> TransType."Rate Source") then
            Error(RateDefinitionErr, Template.ID, TransType.Code);
    end;

    local procedure ChargeAlreadyPosted(PoolCode: Code[20]; TransTypeCode: Code[10]; SourceDocumentNo: Code[20]; SourceLineNo: Integer): Boolean
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    /// here); System = no rate.
    /// </summary>
    procedure ResolveRate(var Template: Record "TAC Pool Charge Template"; var Ctx: Record "TAC Pool Charge Context" temporary): Decimal
    var
    ...
    begin
        case Template."Rate Source" of
            Template."Rate Source"::Fixed:
                exit(Template.Rate);
            Template."Rate Source"::Customer:
                begin
                    if not ShiptoAddress.Get(Ctx."Customer No.", Ctx."DC Code") then
                        exit(0);
            ...
            Template."Rate Source"::Calculated:
                exit(0); // FR — computed in the close (OI-04).
            Template."Rate Source"::System:
                exit(0);
        end;
    end;

    local procedure MeasureBase(RateType: Enum "TAC Pool Rate Type"; var Ctx: Record "TAC Pool Charge Context" temporary): Decimal
    begin
        case RateType of
            RateType::Kg:
                exit(Ctx.Kgs);
    ...
    local procedure SupplierTypeLetter(SupplierType: Enum "TAC Grower Pool Type"): Code[10]
    begin
        case SupplierType of
            SupplierType::Internal:
                exit('I');
            SupplierType::External:
                exit('E');
            SupplierType::"Contract Pack":
                exit('G');
        end;
    end;

    local procedure GrowerTypeLetter(GrowerType: Enum "TAC Grower Type"): Code[10]
    begin
        case GrowerType of
            GrowerType::Internal:
                exit('I');
            GrowerType::External:
                exit('E');
            GrowerType::"Contract Pack":
                exit('G');
            GrowerType::Consolidator:
                exit('C');
            GrowerType::"Fixed FruitBank":
                exit('F');
        end;
    end;
}
