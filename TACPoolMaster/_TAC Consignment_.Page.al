        ...
        area(Navigation)
        {
            action(CarrierManifests)
            {
                ...
                begin
                    ...
                    FreightLine.FindSet();
                    repeat
                        FilterText += '|' + FreightLine."Manifest No.";
                    until FreightLine.Next() = 0;
                    if CopyStr(FilterText, 1, 1) = '|' then
                        FilterText := CopyStr(FilterText, 2);
                    Manifest.SetFilter("No.", FilterText);
                    if Manifest.FindSet() then
                        Page.Run(Page::"TAC Carrier Manifests", Manifest);
                end;
            }
            action(Dimensions)
            {
