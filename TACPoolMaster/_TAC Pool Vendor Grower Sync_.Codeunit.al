codeunit 50286 "TAC Pool Vendor Grower Sync"
{
    ...
    begin
        ...
        if GrowerDimensionCode = '' then GrowerCode:=''
        else if DefaultDimension.Get(Database::Vendor, Vendor."No.", GrowerDimensionCode)then GrowerCode:=DefaultDimension."Dimension Value Code";
        if Vendor."TAC Grower Code" = GrowerCode then exit;
    ...
    begin
        if Vendor.Get(VendorNo)then SyncVendor(Vendor);
    end;
    procedure SyncAllVendors()
    var
        Vendor: Record Vendor;
    begin
        if Vendor.FindSet()then repeat SyncVendor(Vendor);
            until Vendor.Next() = 0;
    end;
    ...
    var
        GrowerDimensionCode: Code[20];
    begin
        if DefaultDimension."Table ID" <> Database::Vendor then exit;
        GrowerDimensionCode:=ConfiguredGrowerDimensionCode();
        if GrowerDimensionCode = '' then exit;
        if DefaultDimension."Dimension Code" <> GrowerDimensionCode then exit;
        SyncVendorNo(DefaultDimension."No.");
    end;
    local procedure ConfiguredGrowerDimensionCode(): Code[20]var
        PoolSetup: Record "TAC Pool Setup";
    begin
        if not PoolSetup.Get()then exit('');
