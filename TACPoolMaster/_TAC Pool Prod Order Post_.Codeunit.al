codeunit 50271 "TAC Pool Prod Order Post"
{
    ...
    begin
        ...
        if HasLegacyHeaderTR(ProductionOrder."No.")then exit;
        if not HasPackedOutput(ProductionOrder."No.", OutputItemNo)then exit;
        if not HasConsumedBinInput(ProductionOrder."No.")then exit;
        ProdOrderLine.SetRange(Status, ProdOrderLine.Status::Finished);
        ProdOrderLine.SetRange("Prod. Order No.", ProductionOrder."No.");
        if ProdOrderLine.FindSet()then repeat RunCloseFromProductionOrderLine(ProductionOrder, ProdOrderLine, PostingDate);
 ...
 VarietyCode, GradeCode, SizeCode, GrowerCode, GrowerPoolType, PackTypeCode, PackTypeCategoryCode);
    ...
    procedure IsPackedItemNo(ItemNo: Code[20]): Boolean var
        Prefix: Code[20];
    begin
    ...
    procedure PackedItemPrefix(): Code[20]begin
        GetSetup();
    ...
    /// </summary>
    procedure HasPackedOutput(ProductionOrderNo: Code[20]; var OutputItemNo: Code[20]): Boolean var
        ItemLedgerEntry: Record "Item Ledger Entry";
    begin
        OutputItemNo:='';
        if PackedItemPrefix() = '' then exit(false);
        FilterOrderEntries(ItemLedgerEntry, ProductionOrderNo, ItemLedgerEntry."Entry Type"::Output);
        if ItemLedgerEntry.FindSet()then repeat if IsPackedItemNo(ItemLedgerEntry."Item No.")then begin
    ...
    /// </summary>
    procedure AlreadyPooled(ProductionOrderNo: Code[20]): Boolean var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        if ProductionOrderNo = '' then exit(false);
    ...
    local procedure AlreadyPooledLine(ProductionOrderNo: Code[20]; ProductionOrderLineNo: Integer): Boolean var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    local procedure HasLegacyHeaderTR(ProductionOrderNo: Code[20]): Boolean var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetCurrentKey("Source Document No.", "Source Line No.", "Trans Type Code");
        PoolLedgerEntry.SetRange("Source Document No.", ProductionOrderNo);
    ...
    procedure HasPackedOutputLine(ProductionOrderNo: Code[20]; ProductionOrderLineNo: Integer; var OutputItemNo: Code[20]): Boolean var
        ItemLedgerEntry: Record "Item Ledger Entry";
    begin
        OutputItemNo:='';
    ...
    procedure GetPackedOutputLine(ProductionOrderNo: Code[20]; ProductionOrderLineNo: Integer; var OutputItemNo: Code[20]): Boolean begin
    ...
    begin
        ...
        FilterOrderEntries(ItemLedgerEntry, ProductionOrderNo, ItemLedgerEntry."Entry Type"::Output);
        if ItemLedgerEntry.FindSet()then repeat if IsPackedItemNo(ItemLedgerEntry."Item No.")then begin
                    if OutputItemNo = '' then OutputItemNo:=ItemLedgerEntry."Item No.";
    ...
    procedure CountOutputEntries(ProductionOrderNo: Code[20]): Integer var
        ItemLedgerEntry: Record "Item Ledger Entry";
    begin
        FilterOrderEntries(ItemLedgerEntry, ProductionOrderNo, ItemLedgerEntry."Entry Type"::Output);
    ...
    /// </summary>
    procedure OutputItemList(ProductionOrderNo: Code[20])List: Text var
        ItemLedgerEntry: Record "Item Ledger Entry";
        Seen: Dictionary of[Code[20], Boolean];
    begin
        FilterOrderEntries(ItemLedgerEntry, ProductionOrderNo, ItemLedgerEntry."Entry Type"::Output);
        if ItemLedgerEntry.FindSet()then repeat if not Seen.ContainsKey(ItemLedgerEntry."Item No.")then begin
    ...
    local procedure TrayEquivalents(ItemNo: Code[20]; BaseQty: Decimal): Decimal var
        QtyPer: Decimal;
    begin
        QtyPer:=ItemUoMQtyPer(ItemNo, TrayEquivUoMCode());
        if QtyPer = 0 then QtyPer:=1;
        exit(BaseQty / QtyPer);
    end;
    local procedure GetConsumedBins(ProductionOrderNo: Code[20]): Decimal var
        ItemLedgerEntry: Record "Item Ledger Entry";
        Item: Record Item;
        BinUoM: Code[10];
        TotalBins: Decimal;
    begin
        ...
        if ItemLedgerEntry.FindSet()then repeat // Only count the bin input, not packaging components.
                if Item.Get(ItemLedgerEntry."Item No.")then if Item."Base Unit of Measure" = BinUoM then TotalBins+=Abs(ItemLedgerEntry.Quantity); // consumption is negative
            until ItemLedgerEntry.Next() = 0;
        exit(TotalBins);
    end;
    procedure HasConsumedBinInput(ProductionOrderNo: Code[20]): Boolean begin
    ...
    /// <summary>Public read-only API for previews and reconciliation.</summary>
    procedure IsOriginalPackRun(ProductionOrderNo: Code[20]): Boolean begin
    ...
    local procedure ItemUoMQtyPer(ItemNo: Code[20]; UoMCode: Code[10]): Decimal var
        ItemUnitOfMeasure: Record "Item Unit of Measure";
    begin
        if(UoMCode <> '') and ItemUnitOfMeasure.Get(ItemNo, UoMCode)then exit(ItemUnitOfMeasure."Qty. per Unit of Measure");
        exit(0);
    end;
    // --- configured units of measure ---
    procedure TrayEquivUoMCode(): Code[10]begin
        GetSetup();
        exit(RequiredUoM(PoolSetup."Tray Equiv. UoM Code", PoolSetup.FieldCaption("Tray Equiv. UoM Code")));
    end;
    procedure KgUoMCode(): Code[10]begin
        GetSetup();
        exit(RequiredUoM(PoolSetup."Kg UoM Code", PoolSetup.FieldCaption("Kg UoM Code")));
    end;
    procedure BinUoMCode(): Code[10]begin
        GetSetup();
    ...
    procedure UomSetupComplete(): Boolean begin
        GetSetup();
        exit((PoolSetup."Tray Equiv. UoM Code" <> '') and (PoolSetup."Kg UoM Code" <> '') and (PoolSetup."Bin UoM Code" <> ''));
    end;
    ...
    local procedure RequiredUoM(UoMCode: Code[10]; SetupFieldCaption: Text): Code[10]begin
        if UoMCode = '' then Error(UomNotSetErr, SetupFieldCaption);
        exit(UoMCode);
    end;
    // --- writing ---
    ...
    begin
        if AlreadyPooledLine(ProductionOrder."No.", ProductionOrderLine."Line No.")then exit;
    ...
    local procedure CreatePoolLedgerFromProdOrder(var ProdOrderLine: Record "Prod. Order Line"; ChargeType: Record "TAC Pool Charge Type"; GrossPoolValue: Decimal; PostingDate: Date)
    var
    ...
    begin
        ...
        case ChargeType."Rate Type" of ChargeType."Rate Type"::Unit: PoolLedgerEntry.Amount:=-PoolLedgerEntry.Quantity * ResolvePoolChargeRate(PostingDate, ChargeType, Vendor."Grower Pool Type", Pool."Grower No.", Pool."Variety Code");
        ChargeType."Rate Type"::Kg: PoolLedgerEntry.Amount:=-PoolLedgerEntry."Quantity (Kg)" * ResolvePoolChargeRate(PostingDate, ChargeType, Vendor."Grower Pool Type", Pool."Grower No.", Pool."Variety Code");
        ChargeType."Rate Type"::Bin: PoolLedgerEntry.Amount:=-GetConsumedBins(ProdOrder."No.") * ResolvePoolChargeRate(PostingDate, ChargeType, Vendor."Grower Pool Type", Pool."Grower No.", Pool."Variety Code");
        ChargeType."Rate Type"::Value: PoolLedgerEntry.Amount:=-GrossPoolValue * ResolvePoolChargeRate(PostingDate, ChargeType, Vendor."Grower Pool Type", Pool."Grower No.", Pool."Variety Code") / 100;
        end;
    ...
    begin
        if(PoolLedgerEntry.Amount = 0) or (PoolLedgerEntry."Charge Type Code" = '')then exit;
    ...
    procedure ResolvePoolChargeRate(AppliedDate: Date; ChargeType: Record "TAC Pool Charge Type"; GrowerPoolType: Enum "TAC Grower Pool Type"; GrowerNo: Code[20]; VarietyCode: Code[20]): Decimal var
    ...
    begin
        ...
        case ChargeType."Rate Type" of ChargeType."Rate Type"::Unit: ChargeRate.SetRange("Rate Type", ChargeRate."Rate Type"::Units);
        ChargeType."Rate Type"::Kg: ChargeRate.SetRange("Rate Type", ChargeRate."Rate Type"::Kilograms);
        ChargeType."Rate Type"::Bin: ChargeRate.SetRange("Rate Type", ChargeRate."Rate Type"::Bins);
        ChargeType."Rate Type"::Value: ChargeRate.SetRange("Rate Type", ChargeRate."Rate Type"::"Value (%)");
        else
            exit(0);
        end;
        ...
        if ChargeRate.FindLast()then exit(ChargeRate.Rate);
        exit(0);
    end;
    ...
    procedure CalculateNormalisedKg(ItemNo: Code[20]; Quantity: Decimal): Decimal var
        Item: Record Item;
