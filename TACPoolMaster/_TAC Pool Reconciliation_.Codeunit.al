...
{
    procedure RunReconciliation(PoolGroupID: Integer): Integer
    var
    ...
    begin
        ...
        if ReconLine.FindSet(true) then
            repeat
                CreateMissingLine(Reconciliation, ReconLine);
        ...
        if ProductionOrderLine.FindSet() then
            repeat
                if ProductionOrder.Get(ProductionOrderLine.Status, ProductionOrderLine."Prod. Order No.") then begin
                    if not PoolProdOrderPost.GetPackedOutputLine(ProductionOrder."No.", ProductionOrderLine."Line No.", OutputItemNo) then
                        continue;
                    if not PoolProdOrderPost.IsOriginalPackRun(ProductionOrder."No.") then
                        continue;
                    ...
                    if not PoolGroup.FindFirst() or (PoolGroup."Pool Group ID" <> Reconciliation."Pool Group ID") then
                        continue;
                    ...
                    if Evaluation.FindSet() then
                        repeat
                            if Evaluation.Result = Evaluation.Result::Expected then
                                AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::"Production Output", ProductionOrder."No.", ProductionOrderLine."Line No.", ProductionOrderLine.SystemId, Reconciliation."Pool Group ID", PoolCode, Evaluation."Trans Type Code", 1, 0, 0, Evaluation."Expected Amount", Evaluation."GST Amount", Evaluation."Template ID", 'Run Close charge.')
                            else
                                if Evaluation.Mandatory then
                                    AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::"Production Output", ProductionOrder."No.", ProductionOrderLine."Line No.", ProductionOrderLine.SystemId, Reconciliation."Pool Group ID", PoolCode, Evaluation."Trans Type Code", 1, 0, 0, 0, 0, Evaluation."Template ID", Evaluation.Details);
    ...
    begin
        ...
        if Expense.FindSet() then
            repeat
                Detail.SetRange("Expense ID", Expense."Expense ID");
                if Detail.FindSet() then
                    repeat
                        AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::Expense, Format(Expense."Expense ID"), Detail."Line No.", Detail.SystemId, Reconciliation."Pool Group ID", Detail."Pool Code", Expense."Trans Type", 1, 0, 0, Detail.Amount, 0, 0, 'Posted expense detail.');
                    until Detail.Next() = 0;
    ...
    begin
        ...
        if Consignment.FindSet() then
            repeat
                ConsignmentLine.SetRange("Consignment No.", Consignment."Consignment No.");
                if ConsignmentLine.FindSet() then
                    repeat
                        if not TryResolveConsignmentDimensions(ConsignmentLine, Season, PoolWeek, Variety, Grade, Size, Grower, GrowerPoolType) then
                            continue;
                        PoolGroup.SetRange("Pool Week Code", CopyStr(PoolWeek, 1, MaxStrLen(PoolGroup."Pool Week Code")));
                        PoolGroup.SetRange("Grower Pool Type", GrowerPoolType);
                        if not PoolGroup.FindFirst() or (PoolGroup."Pool Group ID" <> Reconciliation."Pool Group ID") then
                        ...
                        if Evaluation.FindSet() then
                            repeat
                                if Evaluation.Result = Evaluation.Result::Expected then
                                    ...
                                    if Evaluation.Mandatory then
                                        AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::Consignment, Consignment."Consignment No.", ConsignmentLine."Line No.", ConsignmentLine.SystemId, Reconciliation."Pool Group ID", Pool."Pool Code", Evaluation."Trans Type Code", 1, 0, 0, 0, 0, Evaluation."Template ID", Evaluation.Details);
                            until Evaluation.Next() = 0;
    ...
    var
        ...
        GrowerPoolType: Enum "TAC Grower Pool Type";
    begin
        if not Setup.Get() or not Setup."Allow Invoice Credits" then
            exit;
        ...
        if SalesCrMemoLine.FindSet() then
            repeat
                ...
                if AppliedInvoiceNo = '' then
                    continue;
                InvoicePostState.SetRange("Posted Invoice No.", AppliedInvoiceNo);
                if InvoicePostState.IsEmpty() then
                    continue;
                if not TryResolveCreditDimensions(SalesCrMemoLine, Season, PoolWeek, Variety, Grade, Size, Grower, GrowerPoolType, PackType, PackTypeCategory) then
                    continue;
                PoolGroup.SetRange("Pool Week Code", CopyStr(PoolWeek, 1, MaxStrLen(PoolGroup."Pool Week Code")));
                PoolGroup.SetRange("Grower Pool Type", GrowerPoolType);
                if not PoolGroup.FindFirst() or (PoolGroup."Pool Group ID" <> Reconciliation."Pool Group ID") then
                ...
                if Evaluation.FindSet() then
                    repeat
                        if Evaluation.Result = Evaluation.Result::Expected then
                            ...
                            if Evaluation.Mandatory then
                                AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::"Sales Credit Memo", SalesCrMemoHeader."No.", SalesCrMemoLine."Line No.", SalesCrMemoLine.SystemId, Reconciliation."Pool Group ID", Pool."Pool Code", Evaluation."Trans Type Code", 1, 0, 0, 0, 0, Evaluation."Template ID", Evaluation.Details);
                    until Evaluation.Next() = 0;
        ...
        if SalesInvoiceLine.FindSet() then
            repeat
                if not SalesInvoiceHeader.Get(SalesInvoiceLine."Document No.") then
                    continue;
                if not FindConsignmentLine(SalesInvoiceLine, ConsignmentLine) then
                    continue;
                if not TryResolveInvoiceDimensions(ConsignmentLine, Season, PoolWeek, Variety, Grade, Size, Grower, GrowerPoolType, PackType, PackTypeCategory) then
                    continue;
                PoolGroup.SetRange("Pool Week Code", CopyStr(PoolWeek, 1, MaxStrLen(PoolGroup."Pool Week Code")));
                PoolGroup.SetRange("Grower Pool Type", GrowerPoolType);
                if not PoolGroup.FindFirst() or (PoolGroup."Pool Group ID" <> Reconciliation."Pool Group ID") then
                ...
                if SalesInvoiceLine."Original Quantity" <> 0 then
                    OriginalQuantity := SalesInvoiceLine."Original Quantity"
                else
                ...
                if Evaluation.FindSet() then
                    repeat
                        if Evaluation.Result = Evaluation.Result::Expected then
                            AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::"Sales Invoice", SalesInvoiceHeader."No.", SalesInvoiceLine."Line No.", SalesInvoiceLine.SystemId, Reconciliation."Pool Group ID", Pool."Pool Code", Evaluation."Trans Type Code", 1, 0, 0, Evaluation."Expected Amount", Evaluation."GST Amount", Evaluation."Template ID", 'Invoice Post charge.')
                        else
                            if Evaluation.Mandatory then
                                AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::"Sales Invoice", SalesInvoiceHeader."No.", SalesInvoiceLine."Line No.", SalesInvoiceLine.SystemId, Reconciliation."Pool Group ID", Pool."Pool Code", Evaluation."Trans Type Code", 1, 0, 0, 0, 0, Evaluation."Template ID", Evaluation.Details);
                    until Evaluation.Next() = 0;
            until SalesInvoiceLine.Next() = 0;
    end;

    local procedure FindConsignmentLine(var SalesInvoiceLine: Record "Sales Invoice Line"; var ConsignmentLine: Record "TAC Consignment Line"): Boolean
    var
    ...
    begin
        ...
        if SalesInvoiceLine."Original Item No." <> '' then
            ItemNo := SalesInvoiceLine."Original Item No."
        else
        ...
        if ItemNo <> '' then
            ConsignmentLine.SetRange("Item No.", ItemNo);
    ...
    local procedure ResolveInvoiceQuantityKg(var SalesInvoiceLine: Record "Sales Invoice Line"; var ConsignmentLine: Record "TAC Consignment Line"; OriginalQuantity: Decimal): Decimal
    begin
        if (ConsignmentLine.Quantity <> 0) and (ConsignmentLine."Quantity (Kg)" <> 0) then
            exit((ConsignmentLine."Quantity (Kg)" / ConsignmentLine.Quantity) * OriginalQuantity);
    ...
    begin
        ...
        if Adjustment.FindSet() then
            repeat
                AddExpected(Reconciliation, Enum::"TAC Pool Recon Source Type"::Adjustment, Format(Adjustment."Adjustment ID"), 0, Adjustment.SystemId, Reconciliation."Pool Group ID", Adjustment."From Pool Code", 'TRD', 4, 0, -Adjustment.Kgs, 0, 0, 0, 'Posted adjustment out.');
    ...
    begin
        ...
        if PoolLedgerEntry.FindSet() then
            repeat
                ReconLine."Actual Ledger Amount" += PoolLedgerEntry.Amount;
            until PoolLedgerEntry.Next() = 0;
        if ReconLine."Actual Ledger Entry Count" > 0 then
            ReconLine.Status := ReconLine.Status::Found
        else
            ReconLine.Status := ReconLine.Status::Missing;
        if (PoolCode = '') or (PoolGroupID = 0) then
    ...
    begin
        ...
        if ReconLine."Source Type" = ReconLine."Source Type"::Adjustment then begin
            ...
            if ReconLine."Expected Transaction Type" = 'TRD' then begin
                ...
                if not TryPostAdjustmentEntry(PoolAdjustmentPost, Adjustment, 'TRA', Adjustment."To Pool Code", Adjustment.Kgs) then begin
                    ReconLine.Status := ReconLine.Status::Error;
    ...
    begin
        ...
        if GrowerPoolType = GrowerPoolType::External then
            Context."Grower Type" := Context."Grower Type"::External
        else
            if GrowerPoolType = GrowerPoolType::"Contract Pack" then
                Context."Grower Type" := Context."Grower Type"::"Contract Pack"
            else
    ...
    local procedure NextReconLineNo(ReconciliationID: Integer): Integer
    var
        ReconLine: Record "TAC Pool Recon Line";
    begin
        ReconLine.SetRange("Reconciliation ID", ReconciliationID);
        if ReconLine.FindLast() then
            exit(ReconLine."Line No." + 10000);
        exit(10000);
    end;
