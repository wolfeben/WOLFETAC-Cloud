codeunit 50277 "TAC Pool Subscribers"
{
    ...
    local procedure OnRunOnBeforeCommit_C5704(
    ...
    begin
        ...
        if TransferShipmentHeader."No." <> '' then
            PoolConsignmentPost.ProcessPostedShipment(TransferShipmentHeader."No.", database::"Transfer Shipment Line");
    end;
    ...
    begin
        ...
        if SalesShipmentHeader."No." <> '' then
            PoolConsignmentPost.ProcessPostedShipment(SalesShipmentHeader."No.", database::"Sales Shipment Line");
        if SalesInvoiceHeader."No." <> '' then
            PoolConsignmentPost.ProcessPostedSalesInvoice(SalesInvoiceHeader."No.");
        if SalesCrMemoHeader."No." <> '' then
            PoolConsignmentPost.ProcessPostedSalesCreditMemo(SalesCrMemoHeader."No.", SalesCrMemoHeader."Applies-to Doc. No.");
    end;
    ...
    local procedure OnAfterPostItemJnlLine_C22(var ItemJournalLine: Record "Item Journal Line";
    ...
    begin
        VendorGrowerSync.SyncFromDefaultDimension(Rec);
        if (xRec."Table ID" = Database::Vendor) and (xRec."Dimension Code" <> Rec."Dimension Code") then
            VendorGrowerSync.SyncFromDefaultDimension(xRec);
    end;
    ...
    var
        VendorGrowerSync: Codeunit "TAC Pool Vendor Grower Sync";
    begin
        if Rec."Grower Dimension Code" <> xRec."Grower Dimension Code" then
            VendorGrowerSync.SyncAllVendors();
    end;
...
*/
    ...
    local procedure OnAfterChangeStatusOnProdOrder_C5407(var ProdOrder: Record "Production Order";
                                                        var ToProdOrder: Record "Production Order";
    ...
    begin
        if NewStatus = NewStatus::Finished then
            PoolProdOrderPost.RunCloseFromProductionOrder(ProdOrder, NewPostingDate);
    end;
    ...
    local procedure OnAfterReleaseSalesDoc_C414(
        var SalesHeader: Record "Sales Header";
        PreviewMode: Boolean;
    ...
    begin
        if not Consignment.Get(SalesHeader."DIY_Consignment No.") then
            Consignment.InitiateFromSalesOrder(SalesHeader);
    end;
    ...
    var
        Consignment: Record "TAC Consignment Header";
    begin
        if not Consignment.Get(TransferHeader."DIY_Consignment No.") then
            Consignment.InitiateFromTransfer(TransferHeader);
    end;
