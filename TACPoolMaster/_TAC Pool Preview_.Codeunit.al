codeunit 50283 "TAC Pool Preview"
{
    ...
    begin
        ...
        repeat PreviewLine(ProductionOrder, ProductionOrderLine, Preview);
    ...
    begin
        ...
        if HasBlockingIssue then exit;
        if not ResolveTarget(PoolWeek, GrowerPoolType, Variety, Grade, Size, PoolGroupID, PoolGroupCode, PoolCode, ProductionOrder."No.", ProductionOrderLine."Line No.", OutputItemNo, Season, GrowerCode, PackType, PackTypeCategory, Units, Kgs, Preview)then exit;
        ...
        if Evaluation.FindSet()then repeat case Evaluation.Result of Evaluation.Result::Expected: begin
                ...
                Evaluation.Result::Calculated: AddIssue(Preview, ProductionOrder."No.", ProductionOrderLine."Line No.", OutputItemNo, PoolGroupCode, PoolCode, PoolWeek, Season, Variety, Grade, Size, GrowerCode, GrowerPoolType, PackType, PackTypeCategory, Units, Kgs, Preview.Severity::Information, StrSubstNo('%1 is calculated freight and is not a Run Close transaction.', Evaluation."Trans Type Code"));
                else if Evaluation.Mandatory then AddIssue(Preview, ProductionOrder."No.", ProductionOrderLine."Line No.", OutputItemNo, PoolGroupCode, PoolCode, PoolWeek, Season, Variety, Grade, Size, GrowerCode, GrowerPoolType, PackType, PackTypeCategory, Units, Kgs, Preview.Severity::Blocking, Evaluation.Details)
                    else
    ...
    begin
        ...
        if not Complete then exit(false);
        case UpperCase(PoolTypeValue)of 'I': GrowerPoolType:=GrowerPoolType::Internal;
        'E': GrowerPoolType:=GrowerPoolType::External;
        'G': GrowerPoolType:=GrowerPoolType::"Contract Pack";
        else
        begin
    ...
    begin
        ...
        if PoolGroup.FindFirst()then begin
            ...
            if Pool.FindFirst()then PoolCode:=Pool."Pool Code"
            else
    ...
    begin
        ...
        if GrowerPoolType = GrowerPoolType::External then ChargeContext."Grower Type":=ChargeContext."Grower Type"::External
        else if GrowerPoolType = GrowerPoolType::"Contract Pack" then ChargeContext."Grower Type":=ChargeContext."Grower Type"::"Contract Pack"
            else
    ...
    local procedure LedgerEntryExists(PoolCode: Code[20]; SourceDocumentNo: Code[20]; SourceLineNo: Integer; TransTypeCode: Code[10]): Boolean var
    ...
    local procedure FindProductionOrder(ProductionOrderNo: Code[20]; var ProductionOrder: Record "Production Order"): Boolean begin
    ...
    begin
        ...
        if Severity = Preview.Severity::Blocking then Preview.Outcome:=Preview.Outcome::"Needs Attention"
        else
    ...
    begin
        ...
        if Preview.FindSet()then repeat Preview."Expected Line Charge Total":=ChargeTotal;
                Preview.Modify();
    ...
    local procedure NextPreviewEntryNo(var Preview: Record "TAC Pool Charge Preview" temporary): Integer begin
        Preview.Reset();
        if Preview.FindLast()then exit(Preview."Entry No." + 1);
        exit(1);
    end;
}
