codeunit 50273 "TAC Pool Group Close"
{
    ...
    var
        PPCodeTok: Label 'PP', Locked = true;
    ...
    begin
        PoolGroup.Get(PoolGroupID);
        if PoolGroup.Status = PoolGroup.Status::Closed then
            Error(ResumableCloseStatusErr, PoolGroup."Pool Group Code");
        ...
        if not PoolPaymentHeader.FindLast() then
            Error(NoResumablePaymentErr, PoolGroup."Pool Group Code");
        ...
        // a second time.
        if (PoolPaymentHeader."Payment Type" = PoolPaymentHeader."Payment Type"::Provisional) and
           (PoolGroup.Status = PoolGroup.Status::"Provisionally Closed") then
            Error(NoResumablePaymentErr, PoolGroup."Pool Group Code");
    ...
    begin
        ...
        if PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::"Contract Pack" then
            Error(GTypeBlockErr, PoolGroup."Pool Group Code");
        if PoolGroup.Status = PoolGroup.Status::Closed then
            Error(ClosedStatusErr, PoolGroup."Pool Group Code");
        if (PaymentType = PaymentType::Provisional) and (PoolGroup.Status = PoolGroup.Status::"Provisionally Closed") then
            ; // a further provisional is allowed up to the limit (V2)
        if (PaymentType = PaymentType::Provisional) and (PoolGroup.Status = PoolGroup.Status::Closed) then
            Error(ProvisionalAfterFinalErr, PoolGroup."Pool Group Code");

        // V2 — provisional limit (2 for Internal, 1 otherwise — confirm split).
        if PaymentType = PaymentType::Provisional then
            if PoolGroup."Provisional Close Count" >= MaxProvisionalCloses(PoolGroup) then
                Error(ProvisionalLimitErr, PoolGroup."Pool Group Code", MaxProvisionalCloses(PoolGroup));
    ...
    local procedure MaxProvisionalCloses(var PoolGroup: Record "TAC Pool Group Header"): Integer
    begin
        if PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::Internal then
            exit(2);
        exit(1);
    end;
    ...
    begin
        ...
        if not PoolExpenseHeader.IsEmpty() then
            Error(UnpostedExpenseErr, PoolGroup."Pool Group Code");
    end;
        ...
        if TransType.FindSet() then
            repeat
                if PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::External then
                    GLAccountNo := TransType."GL Account External"
                else
                    GLAccountNo := TransType."GL Account Internal";
                if GLAccountNo = '' then
                    Error(MandatoryGLErr, TransType."Code");
    ...
    begin
        ...
        foreach GrowerCode in Growers do
            if not GrowerMgt.FindVendorNoForGrower(GrowerCode, VendorNo) then
                Error(NoGrowerVendorErr, PoolGroup."Pool Group Code", GrowerCode);
    end;
    ...
    begin
        if not PoolSetup.Get() then
            Error(PostingSetupErr, PoolGroup."Pool Group Code", 'Pool Payment Setup');
        if PoolSetup."Pool Charge Clearing Account" = '' then
            Error(PostingSetupErr, PoolGroup."Pool Group Code", PoolSetup.FieldCaption("Pool Charge Clearing Account"));
        if PoolSetup."Grower Posting Group" = '' then
            Error(PostingSetupErr, PoolGroup."Pool Group Code", PoolSetup.FieldCaption("Grower Posting Group"));
        if (PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::External) and
           (PoolSetup."Grower Ext. Posting Group" = '') then
            Error(PostingSetupErr, PoolGroup."Pool Group Code", PoolSetup.FieldCaption("Grower Ext. Posting Group"));
    end;
    ...
    begin
        ...
        if Pool.FindSet() then
            repeat
                BuildPoolContext(ChargeContext, PoolGroup, Pool, 0);
    ...
    begin
        ...
        if Pool.FindSet() then
            repeat
                BuildPoolContext(ChargeContext, PoolGroup, Pool, PoolPaymentID);
    ...
    begin
        ...
        if TransType.FindSet() then
            repeat
                if TransType."Prorata to Grower Level" = TransType."Prorata to Grower Level"::PoolGroup then
                    ProrateGroupScope(PoolGroup, TransType."Code", PoolPaymentID)
                else begin
                    Pool.SetRange("Pool Group ID", PoolGroup."Pool Group ID");
                    if Pool.FindSet() then
                        repeat
                            ProratePoolScope(PoolGroup, Pool, TransType."Code", PoolPaymentID);
                        until Pool.Next() = 0;
                end;
            until TransType.Next() = 0;
    end;
    ...
    begin
        ...
        for Idx := 1 to Growers.Count() do begin
            ...
            if Idx < Growers.Count() then
                Share := Round(Total * GetGrowerAdjKgs(Pool."Pool Code", GrowerCode) / DenomKgs)
            else
    ...
    begin
        ...
        for Idx := 1 to Growers.Count() do begin
            GrowerCode := Growers.Get(Idx);
            if Idx < Growers.Count() then
                Share := Round(Total * GetGrowerGroupAdjKgs(PoolGroup."Pool Group ID", GrowerCode) / DenomKgs)
            else
    ...
    begin
        if PaymentType = PaymentType::Final then
            TransTypeCode := PPCodeTok
        else
        ...
        if Pool.FindSet() then
            repeat
                // The pool's value BEFORE any payment, so the percentage is a
                ...
                if (PoolAdjKgs <> 0) and (Distributable <> 0) then begin
                    ...
                    for Idx := 1 to Growers.Count() do begin
                        ...
                        if Idx < Growers.Count() then
                            Payment := Round(Distributable * GetGrowerAdjKgs(Pool."Pool Code", GrowerCode) / PoolAdjKgs * -1)
                        else
    ...
    /// </summary>
    local procedure TargetPaymentPct(var PoolGroup: Record "TAC Pool Group Header"; PaymentType: Enum "TAC Pool Payment Type"): Decimal
    var
        PoolSetup: Record "TAC Pool Setup";
    begin
        if PaymentType = PaymentType::Final then
        ...
        if Pool.FindSet() then
            repeat
                ...
                for Idx := 1 to Growers.Count() do begin
                    ...
                    TransType.FindSet();
                    repeat
                        // Rate x grower payment magnitude. ResolveRate keeps the
                        // rate-source logic in the engine (ADR-003).
                        WriteGrowerCharge(PoolGroup, Pool."Pool Code", GrowerCode, TransType."Code",
                            GrowerValueCharge(ChargeEngine, TransType, Pool."Pool Code", GrowerCode), PoolPaymentID);
                    until TransType.Next() = 0;
                end;
            until Pool.Next() = 0;
    end;

    local procedure GrowerValueCharge(var ChargeEngine: Codeunit "TAC Pool Charge Engine"; var TransType: Record "TAC Pool Trans Type"; PoolCode: Code[20]; GrowerCode: Code[20]): Decimal
    var
    ...
    local procedure CreatePaymentHeader(var PoolGroup: Record "TAC Pool Group Header"; PaymentType: Enum "TAC Pool Payment Type"; var BatchName: Code[10]): Integer
    var
        PoolPaymentHeader: Record "TAC Pool Payment Header";
    begin
    ...
    var
        PoolPaymentHeader: Record "TAC Pool Payment Header";
    begin
        if not PoolPaymentHeader.Get(PoolPaymentID) then
            exit;
    ...
    local procedure NextPaymentNo(PoolGroupID: Integer): Integer
    var
        PoolPaymentHeader: Record "TAC Pool Payment Header";
    begin
        PoolPaymentHeader.SetRange("Pool Group ID", PoolGroupID);
    ...
    local procedure GetPoolNet(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    /// </summary>
    local procedure GetPoolValueExclPayments(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    /// </summary>
    local procedure GetPaymentsInPool(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetPoolAdjKgs(PoolCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetGrowerAdjKgs(PoolCode: Code[20]; GrowerCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetGroupAdjKgs(PoolGroupID: Integer): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    local procedure GetGrowerGroupAdjKgs(PoolGroupID: Integer; GrowerCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Group ID", PoolGroupID);
    ...
    local procedure GetPoolLevelChargeTotal(PoolCode: Code[20]; TransTypeCode: Code[10]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure GetGroupLevelChargeTotal(PoolGroupID: Integer; TransTypeCode: Code[10]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Group ID", PoolGroupID);
    ...
    local procedure GetGrowerPaymentInPool(PoolCode: Code[20]; GrowerCode: Code[20]): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
        PoolLedgerEntry.SetRange("Pool Code", PoolCode);
    ...
    local procedure DistinctGrowersInPool(PoolCode: Code[20]): List of [Code[20]]
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
        Growers: List of [Code[20]];
    begin
        ...
        if PoolLedgerEntry.FindSet() then
            repeat
                if not Growers.Contains(PoolLedgerEntry."Grower Code") then
                    Growers.Add(PoolLedgerEntry."Grower Code");
            until PoolLedgerEntry.Next() = 0;
        exit(Growers);
    end;

    local procedure DistinctGrowersInGroup(PoolGroupID: Integer): List of [Code[20]]
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
        Growers: List of [Code[20]];
    begin
