codeunit 50279 "TAC Pool GL Posting"
{
    ...
    var
        // Held at codeunit level so its resolution cache lives for the whole
    ...
    begin
        if not PaymentFinancialEntriesPosted(PoolGroup."Pool Group ID", PoolPaymentID) then
            Error('The financial entries for Pool Payment %1 have not been posted to G/L, so this close cannot be resumed.', PoolPaymentID);
    ...
    local procedure PaymentFinancialEntriesPosted(PoolGroupID: Integer; PoolPaymentID: Integer): Boolean
    var
    ...
    begin
        if not PoolPaymentHeader.Get(PoolPaymentID) then
            exit(false);
        if (PoolPaymentHeader."Pool Group ID" <> PoolGroupID) or
           (PoolPaymentHeader."GL Journal Batch Name" = '') then
            exit(false);
    ...
    begin
        ...
        if PoolLedgerEntry.FindSet() then
            repeat
                if not IsKgMovement(PoolLedgerEntry."Trans Type Code") then begin
                    AggKey := AggregationKey(PoolGroup, PoolLedgerEntry);
                    if AmountByKey.ContainsKey(AggKey) then
                        AmountByKey.Set(AggKey, AmountByKey.Get(AggKey) + PoolLedgerEntry.Amount)
                    else
    ...
    begin
        PoolLedgerEntry.SetRange("Pool Group ID", PoolGroup."Pool Group ID");
        PoolLedgerEntry.SetRange("Posted to G/L", false);
        if PoolLedgerEntry.FindSet(true) then
            repeat
                if not IsKgMovement(PoolLedgerEntry."Trans Type Code") then begin
    ...
    local procedure AggregationKey(var PoolGroup: Record "TAC Pool Group Header"; var PoolLedgerEntry: Record "TAC Pool Ledger Entry"): Text
    var
        GLAccountNo: Code[20];
    begin
        ...
        if IsGrowerSpecific(PoolLedgerEntry."Trans Type Code", PoolLedgerEntry."Grower Code") then
            exit(StrSubstNo('%1|%2', GLAccountNo, PoolLedgerEntry."Grower Code"));
        exit(StrSubstNo('%1|', GLAccountNo));
    end;

    local procedure ResolveGLAccount(var PoolGroup: Record "TAC Pool Group Header"; TransTypeCode: Code[10]; GrowerCode: Code[20]): Code[20]
    var
    ...
    begin
        ...
        if (GrowerCode <> '') and GrowerGLOverride.Get(GrowerCode, TransTypeCode) then
            exit(GrowerGLOverride."Override GL Account");

        if not TransType.Get(TransTypeCode) then
            exit('');
        if PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::External then
            exit(TransType."GL Account External");
        exit(TransType."GL Account Internal");
    end;

    local procedure IsGrowerSpecific(TransTypeCode: Code[10]; GrowerCode: Code[20]): Boolean
    var
        GrowerGLOverride: Record "TAC Grower GL Override";
    begin
    ...
    begin
        ...
        foreach AggKey in AmountByKey.Keys() do begin
            ...
            if Parts.Count >= 2 then
                GrowerCode := CopyStr(Parts.Get(2), 1, MaxStrLen(GrowerCode))
            else
                GrowerCode := '';
            ...
            if GrowerCode <> '' then
                GenJournalLine.Description := CopyStr(StrSubstNo('Pool payment %1', GrowerCode), 1, MaxStrLen(GenJournalLine.Description));
    ...
    begin
        ...
        if PoolLedgerEntry.FindSet() then
            repeat
                if not GrowerList.Contains(PoolLedgerEntry."Grower Code") then
                    GrowerList.Add(PoolLedgerEntry."Grower Code");
            until PoolLedgerEntry.Next() = 0;

        foreach GrowerCode in GrowerList do
            BuildGrowerInvoice(PoolGroup, GrowerCode, PaymentType, PoolPaymentID);
    end;
    ...
    begin
        if not PostingEnabled() then
            exit; // documents require posting-group config (§12) — see header note.

        VendorNo := GrowerMgt.VendorNoForGrower(GrowerCode);
        if FindPostedGrowerDocument(VendorNo, PoolPaymentID, PoolGroup."Pool Group Code") then
            exit;
        ...
        if PurchaseHeader."Document Type" = PurchaseHeader."Document Type"::"Credit Memo" then
            PoolSetup.TestField("Grower Settlement Reason Code");
        if PoolSetup."Grower Settlement Reason Code" <> '' then
            PurchaseHeader.Validate("Reason Code", PoolSetup."Grower Settlement Reason Code");
        ...
        if PurchaseHeader."Document Type" = PurchaseHeader."Document Type"::Invoice then
            PurchaseHeader.Validate("Vendor Invoice No.", PoolPaymentReference(PoolPaymentID, PoolGroup."Pool Group Code"))
        else
        ...
        if PoolGroup."Grower Pool Type" = PoolGroup."Grower Pool Type"::External then begin
            if PoolSetup."Grower Ext. Posting Group" <> '' then
                PurchaseHeader.Validate("Vendor Posting Group", PoolSetup."Grower Ext. Posting Group");
        end else
            if PoolSetup."Grower Posting Group" <> '' then
                PurchaseHeader.Validate("Vendor Posting Group", PoolSetup."Grower Posting Group");
    ...
    local procedure FindPostedGrowerDocument(VendorNo: Code[20]; PoolPaymentID: Integer; PoolGroupCode: Code[20]): Boolean
    begin
        exit(PostedGrowerDocumentNo(VendorNo, PoolPaymentID, PoolGroupCode) <> '');
    end;

    local procedure PostedGrowerDocumentNo(VendorNo: Code[20]; PoolPaymentID: Integer; PoolGroupCode: Code[20]): Code[20]
    var
    ...
    local procedure PoolPaymentReference(PoolPaymentID: Integer; PoolGroupCode: Code[20]): Code[35]
    begin
    ...
    begin
        if (InvoiceNo = '') or not PoolPaymentHeader.Get(PoolPaymentID) then
            exit;

        if StrPos(PoolPaymentHeader."Created Purchase Invoice Nos.", InvoiceNo) <> 0 then
            exit;

        UpdatedInvoiceNos := PoolPaymentHeader."Created Purchase Invoice Nos.";
        if UpdatedInvoiceNos <> '' then
            UpdatedInvoiceNos += ',';
    ...
    local procedure GrowerSettlementAmount(var PoolGroup: Record "TAC Pool Group Header"; GrowerCode: Code[20]; PoolPaymentID: Integer): Decimal
    var
    ...
    begin
        ...
        if Pool.FindSet() then
            repeat
                SettlementAmount += Abs(GrowerPaymentForPool(Pool."Pool Code", GrowerCode, PoolPaymentID));
            until Pool.Next() = 0;
        ...
        if PoolGrowerCharge.FindSet() then
            repeat
                if TransType.Get(PoolGrowerCharge."Trans Type Code") then
                    if not TransType."Suppress on Grower Invoice" then
                        SettlementAmount -= PoolGrowerCharge.Amount;
    ...
    begin
        ...
        PoolGrowerCharge.SetRange("Pool Group ID", PoolGroup."Pool Group ID");
        PoolGrowerCharge.SetRange("Grower Code", GrowerCode);
        if PoolPaymentID <> 0 then
            PoolGrowerCharge.SetRange("Pool Payment ID", PoolPaymentID);
        if PoolGrowerCharge.FindSet() then
            repeat
                if TransType.Get(PoolGrowerCharge."Trans Type Code") then
                    if not TransType."Suppress on Grower Invoice" then begin
                        LineNo += 10000;
    ...
    local procedure AppendPoolPaymentLines(var PurchaseHeader: Record "Purchase Header"; var PoolGroup: Record "TAC Pool Group Header"; GrowerCode: Code[20]; PoolPaymentID: Integer; StartLineNo: Integer): Integer
    var
        Pool: Record "TAC Pool";
    ...
    begin
        ...
        if Pool.FindSet() then
            repeat
                //Amount := GrowerPaymentForPool(Pool."Pool ID", GrowerCode);
    ...
    local procedure GrowerPaymentForPool(PoolCode: Code[20]; GrowerCode: Code[20]; PoolPaymentID: Integer): Decimal
    var
        PoolLedgerEntry: Record "TAC Pool Ledger Entry";
    begin
    ...
    local procedure IsKgMovement(TransTypeCode: Code[10]): Boolean
    begin
    ...
    local procedure PostingEnabled(): Boolean
    var
        PoolSetup: Record "TAC Pool Setup";
    begin
