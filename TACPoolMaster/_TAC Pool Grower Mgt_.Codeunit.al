codeunit 50280 "TAC Pool Grower Mgt"
{
    ...
    /// </summary>
    procedure VendorNoForGrower(GrowerCode: Code[20]): Code[20]
    var
        VendorNo: Code[20];
    begin
        if GrowerCode = '' then
            Error(BlankGrowerErr);
        if not FindVendorNoForGrower(GrowerCode, VendorNo) then
            Error(NoVendorErr, GrowerCode, DimensionMgt.GrowerDimensionCode());
        exit(VendorNo);
    end;
    ...
    /// </summary>
    procedure FindVendorNoForGrower(GrowerCode: Code[20]; var VendorNo: Code[20]): Boolean
    begin
        VendorNo := '';
        if GrowerCode = '' then
            exit(false);
        ...
        if not VendorNoCache.ContainsKey(GrowerCode) then
            VendorNoCache.Set(GrowerCode, FindVendorNo(GrowerCode));
    ...
    procedure GrowerCodeForVendor(VendorNo: Code[20]): Code[20]
    var
        DefaultDimension: Record "Default Dimension";
        GrowerCode: Code[20];
    begin
        if not DefaultDimension.Get(Database::Vendor, VendorNo, DimensionMgt.GrowerDimensionCode()) then
            Error(NoGrowerOnVendorErr, VendorNo, DimensionMgt.GrowerDimensionCode());
    ...
    begin
        ...
        if not DefaultDimension.FindSet() then
            exit('');

        FoundVendorNo := CopyStr(DefaultDimension."No.", 1, MaxStrLen(FoundVendorNo));
        if DefaultDimension.Next() <> 0 then
            Error(AmbiguousVendorErr, GrowerCode, FoundVendorNo, DefaultDimension."No.");
    end;
    ...
    /// </summary>
    procedure LookupGrowerCode(var GrowerCode: Code[20]): Boolean
    var
    ...
    begin
        if GrowerCode = '' then
            exit;
        if not DimensionValue.Get(DimensionMgt.GrowerDimensionCode(), GrowerCode) then
            Error(UnknownGrowerErr, GrowerCode, DimensionMgt.GrowerDimensionCode());
    end;
