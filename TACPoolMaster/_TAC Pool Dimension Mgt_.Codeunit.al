codeunit 50276 "TAC Pool Dimension Mgt"
{
    ...
    procedure ResolveFromProductionOrder(var ProductionOrder: Record "Production Order";
    //var SeasonCode: Code[20]; var PoolWeekCode: Code[20]; 
    ...
    begin
        ...
        GrowerCode := GrowerMgt.GrowerCodeForSourceVendor(ProductionOrder."Grower ID", ProductionOrder."Dimension Set ID");
    end;

    procedure ResolveFromDimensionSet(
        DimensionSetID: Integer;
        //var SeasonCode: Code[20];
        //var PoolWeekCode: Code[20];
        var VarietyCode: Code[20];
        var GradeCode: Code[20];
        var SizeCode: Code[20];
        //var GrowerCode: Code[20];
    ...
    procedure ResolveFromProductionOrderLine(var ProductionOrderLine: Record "Prod. Order Line";
    //var SeasonCode: Code[20]; var PoolWeekCode: Code[20]; 
    ...
    procedure ResolveFromDimensionSetWithPacking(
        DimensionSetID: Integer;
        //var SeasonCode: Code[20]; 
        //var PoolWeekCode: Code[20]; 
        var VarietyCode: Code[20];
        ...
        //var GrowerCode: Code[20];
        var GrowerPoolType: Enum "TAC Grower Pool Type";
        var PackTypeCode: Code[20];
        var PackTypeCategoryCode: Code[20])
    begin
    ...
    procedure HasPoolDimensions(DimensionSetID: Integer): Boolean
    begin
        ...
        if not DimensionSetupComplete() then
            exit(false);
        exit(GetDimensionValue(DimensionSetID, VarietyDimensionCode()) <> '');
    end;

    procedure GetDimensionValue(DimensionSetID: Integer; DimensionCode: Code[20]): Code[20]
    var
        DimSetEntry: Record "Dimension Set Entry";
    begin
        if DimSetEntry.Get(DimensionSetID, DimensionCode) then
            exit(DimSetEntry."Dimension Value Code");
        exit('');
    end;

    procedure MapGrowerPoolType(DimensionValue: Code[20]): Enum "TAC Grower Pool Type"
    begin
        ...
        case UpperCase(DimensionValue) of
            'I':
                exit(Enum::"TAC Grower Pool Type"::Internal);
            'E':
                exit(Enum::"TAC Grower Pool Type"::External);
            'G':
                exit(Enum::"TAC Grower Pool Type"::"Contract Pack");
            else
    ...
    procedure GetConfiguredGrowerDimensionCode(var GrowerDimensionCode: Code[20]): Boolean
    begin
        GetSetup();
    ...
    procedure DimensionSetupComplete(): Boolean
    begin
        GetSetup();
        exit(
            (PoolSetup."Season Dimension Code" <> '') and
            (PoolSetup."Pool Week Dimension Code" <> '') and
            (PoolSetup."Variety Dimension Code" <> '') and
            (PoolSetup."Grade Dimension Code" <> '') and
            (PoolSetup."Size Dimension Code" <> '') and
            (PoolSetup."Grower Dimension Code" <> '') and
            (PoolSetup."Grower Pool Type Dim. Code" <> '') and
            (PoolSetup."Pack Type Dimension Code" <> '') and
            (PoolSetup."Pack Type Category Dim. Code" <> ''));
    end;
    ...
    local procedure RequiredCode(DimensionCode: Code[20]; SetupFieldCaption: Text): Code[20]
    begin
        if DimensionCode = '' then
            Error(DimensionNotSetErr, SetupFieldCaption);
        exit(DimensionCode);
    end;

    local procedure ValidateDimensionValue(DimensionSetID: Integer; DimensionCode: Code[20]; SetupFieldCaption: Text)
    begin
        if GetDimensionValue(DimensionSetID, DimensionCode) = '' then
            Error(DimensionValueMissingErr, SetupFieldCaption);
    end;
}
